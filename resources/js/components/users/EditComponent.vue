                        ...
                        <v-tab-item id="details">
                            ...
                            <v-form @submit.prevent="submit" @reset.prevent="reset" ref="form" lazy-validation id="rolecreate" class="mt-4">
                                <validation-provider v-slot="{errors}" name="名稱" rules="required|max:10">
                                    <v-text-field name="name" v-model="user.name" label="名稱" id="name" :error-messages="errors"></v-text-field>
                                </validation-provider>
                                <validation-provider v-slot="{errors}" name="電子信箱" rules="required|email">
                                    <v-text-field name="email" v-model="user.email" label="電子信箱" id="email" :error-messages="errors"></v-text-field>
                                </validation-provider>
                                <validation-provider v-slot="{errors}" name="角色" rules="required">
                                    <v-select name="role" v-model="user_role" :items="roles" item-text="name" label="角色" chips :error-messages="errors">
